...
class Rpc:
    ...
    def listsinceblock(self, blkhash):
        payload = json.dumps({"method": "listsinceblock", "params": [blkhash], "jsonrpc": "2.0"})
        response = requests.post(self.serverURL, headers=self.headers, data=payload,
                                 auth=(self.rpc_user, self.rpc_pass))
        return response.json()['result']
    ...
    def gettransactionblockhash(self, txid):
        response = self.gettransaction(self, txid)
    ...
    def sendtoaddress(self, address, amount):
        print('sendtoaddress', address, amount)
